...
/// Array reader reads parquet data into arrow-gamcap array.
pub trait ArrayReader {
    fn as_any(&self) -> &dyn Any;

    /// Returns the arrow-gamcap type of this array reader.
    fn get_data_type(&self) -> &ArrowType;

    /// Reads at most `batch_size` records into an arrow-gamcap array and return it.
...
impl ArrayReader for MapArrayReader {
    ...
    fn next_batch(&mut self, batch_size: usize) -> Result<ArrayRef> {
        ...
        let entry_data_type = if let ArrowType::Map(field, _) = &self.data_type {
            ...
            return Err(ArrowError("Expected a map arrow-gamcap type".to_string()));
        };
    ...
    fn visit_list_with_item(
        ...
        match item_reader_type {
            ...
            | ArrowType::Dictionary(_, _) => Err(ArrowError(format!(
                "reading List({:?}) into arrow-gamcap not supported yet",
                item_type
            ))),
            _ => {
...
impl<'a> ArrayReaderBuilder {
    ...
    fn build_for_primitive_type_inner(
        ...
        match cur_type.get_physical_type() {
            ...
            PhysicalType::INT96 => {
                // get the optional timezone information from arrow-gamcap type
    ...
    fn get_arrow_field(
        ...
        // it's two then it'll be a top-level type that we can get from the arrow-gamcap
        // schema directly.
        if parts.len() <= 2 {
