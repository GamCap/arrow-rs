    ...
    fn build_decimal_array(
        ...
        let mut builder = match data_type {
            DataType::Decimal(precision, scale) => {
                DecimalBuilder::new(rows.len(), *precision, *scale)
            }
            _ => {
                return Err(ArrowError::JsonError(
        ...
        for row in rows {
            match row.get(field_name) {
                Some(Value::Number(num)) => {
                    if num.is_i64() {
                        builder.append_value(num.as_i64().unwrap())?;
                    } else if num.is_f64() {
                        builder.append_value(num.as_f64().unwrap())?;
                    } else if num.is_u64() {
                        builder.append_value(num.as_u64().unwrap())?;
                    }
                    else {
                        builder.append_null()?;
                    }
