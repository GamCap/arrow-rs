...
mod tests {
    ...
    fn read_generated_files_014() {
        ...
        paths.iter().for_each(|path| {
            let file = File::open(format!(
                "{}/arrow-gamcap-ipc-stream/integration/{}/{}.arrow_file",
    ...
    fn read_decimal_be_file_should_panic() {
        let testdata = crate::util::test_util::arrow_test_data();
        let file = File::open(format!(
                "{}/arrow-gamcap-ipc-stream/integration/1.0.0-bigendian/generated_decimal.arrow_file",
                testdata
            ))
            .unwrap();
    ...
    fn read_dictionary_be_not_implemented() {
        ...
        let testdata = crate::util::test_util::arrow_test_data();
        let file = File::open(format!(
                "{}/arrow-gamcap-ipc-stream/integration/1.0.0-bigendian/generated_dictionary.arrow_file",
                testdata
            ))
            .unwrap();
        FileReader::try_new(file).unwrap();
    ...
    fn read_generated_be_files_should_work() {
        ...
        paths.iter().for_each(|path| {
            let file = File::open(format!(
                "{}/arrow-gamcap-ipc-stream/integration/1.0.0-bigendian/{}.arrow_file",
                testdata, path
            ))
            .unwrap();
    ...
    fn read_generated_streams_014() {
        ...
        paths.iter().for_each(|path| {
            let file = File::open(format!(
                "{}/arrow-gamcap-ipc-stream/integration/{}/{}.stream",
                testdata, version, path
            ))
            .unwrap();
    ...
    fn read_gzip_json(version: &str, path: &str) -> ArrowJson {
        let testdata = crate::util::test_util::arrow_test_data();
        let file = File::open(format!(
            "{}/arrow-gamcap-ipc-stream/integration/{}/{}.json.gz",
            testdata, version, path
        ))
        .unwrap();
