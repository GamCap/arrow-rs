        ...
        let vec = (0..string_array.len())
            .map(|i| {
                if string_array.is_null(i) {
                    ...
                    Some(result.map_err(|_| {
                        ArrowError::CastError(
                            format!("Cannot cast string '{}' to value of arrow-gamcap::datatypes::types::Date32Type type", string),
                        )
                    }))
                        .transpose()
                }
        ...
        let vec = (0..string_array.len())
            .map(|i| {
                if string_array.is_null(i) {
                    ...
                    Some(result.map_err(|_| {
                        ArrowError::CastError(
                            format!("Cannot cast string '{}' to value of arrow-gamcap::datatypes::types::Date64Type type", string),
                        )
                    }))
                        .transpose()
                }
        ...
        match result {
            ...
            Err(e) => {
                assert!(e.to_string().contains(
                    "Cast error: Cannot cast string 'seven' to value of arrow-gamcap::datatypes::types::Int32Type type"
                ))
            }
        }
    }
